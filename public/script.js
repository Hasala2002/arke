...
const checkLastMessage = (uid) => {
	if ($('#messageList').html() !== '') {
		if (
			$('#messageList .chat-messages:last-child').attr('data-id') === uid
		) {
			return true;
		} else if (
			$('#messageList .chat-images:last-child').attr('data-id') === uid
		) {
			return true;
		} else {
			return false;
...
$('#happy').click(() => {
	...
	$('.emoji-picker-window').hasClass('show')
		? $('#happy ion-icon').attr('name', 'close')
		: $('#happy ion-icon').attr('name', 'happy');
});
...
const userHasSumbmittedName = () => {
	const peer = new Peer(undefined, {
		...
		host: '/',
		port:
			window.location.origin === 'https://arkechat.herokuapp.com'
				? '443'
				: '3000',
	});
	peer.on('open', (id) => {
...
console.log(
	'%c          (ㆆ_ㆆ)',
	'font-size:20px; font-weight: 600; color: #d21f3c;'
);
console.log(
	'%cNothing to see here, silly!',
	'font-size:20px; font-weight: 600; color: #d21f3c;'
);
